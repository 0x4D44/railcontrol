...
## 2. Scope
- Applies to the MSVC/OWLNext 7.0.19 build only; no Borland toolchain is used or supported.
...
| --- | --- | --- |
| **railc_msvc.exe** | Hosts OWLNext windows, menu handlers, dialogs, rendering surfaces, automation hooks. | OWLNext 7.0.19, Win32 GDI |
...
- **Domain Layer**: `RailCore` exposes `IRailEngine` and `ILayoutRepository`, returning immutable snapshots or change events.
...
namespace RailCore {
  ...
  struct SimulationTickResult {
    std::shared_ptr<const WorldDelta> delta; // optional partial state diff since previous tick (nullptr if no change)
  ...
  struct AdvanceOutcome {
    Status status;                    // Ok on success; non-OK with message otherwise
  ...
  public:
    ...
    // Returns status + tick result atomically to avoid ambiguity between
    // transport errors and a zero-change tick.
    virtual AdvanceOutcome Advance(std::chrono::milliseconds dt) = 0;
    // Reset/unload current layout and clear state; engine moves to paused/idle.
...
public:
  ...
  // hwndMain may be nullptr for headless use (tests/CLI harnesses).
...
- `DelaySettings` and `LocoAssignment` are POD structs defined in `railcore/commands.h`, isolating command payloads from UI types. Each captures only serializable data so commands can be replayed in automation runs.
...
- `IRailControllerDelegate` declares callbacks (`OnStatus`, `OnSnapshot`, `OnTick`) that fan updates into specific UI widgets; separating this from `IObserver` lets the controller perform view-model transformations before reaching OWL controls and centralizes UI-side error handling.
...
- Help/assistance actions are handled at the controller/UI layer; the backend does not expose any help-related commands and remains UI-agnostic.
 - Subscriptions: call `Subscribe` during controller initialization before timers start; unsubscribe on shutdown. Re-entrant callbacks into the engine during observer notifications are undefined behavior and may return `StatusCode::Busy`.
...
namespace RailCore {
  ...
  // Help/assistance topics are handled by the controller/UI layer.
...
struct EngineConfig {
  ...
  size_t maxSections {2048};                   // upper bound for sections
  size_t maxRoutes {4096};                     // upper bound for routes
  size_t maxTimetableEntries {8192};           // upper bound for timetable entries
...
- `maxActiveTrains`, `maxSections`, `maxRoutes`, and `maxTimetableEntries` bound pre-allocated pools/tables; exceeding any limit returns `StatusCode::ValidationError` (for configuration time) or `StatusCode::InternalError` (for runtime overflow) and emits diagnostics so limits can be tuned.
...
#### 5.3.5 World Delta Semantics
To minimize UI work and memory pressure, ticks publish an optional delta in addition to full snapshots:
```cpp
namespace RailCore {
  struct EntityDelta {
    uint32_t id;                      // stable entity id
    std::map<std::string, std::string> changedFields; // name->stringified value
  };
  struct WorldDelta {                 // partial state changes since last tick
...
```
- `SimulationTickResult::delta` points to `WorldDelta` when non-empty; it is `nullptr` if nothing meaningful changed. The UI may prefer deltas for incremental updates; for simplicity, initial implementations may set `delta = nullptr` until diffs are available.
- Deltas favor readability and loose coupling over micro-optimizations; when performance matters, introduce typed deltas for hot paths.
#### 5.3.6 Identity & Versioning
- Entity IDs are stable across the lifetime of a loaded layout and are assigned by the repository/loader. IDs are not reused until `LoadLayout` completes a full teardown.
- `WorldState::tickId` monotonically increases per call to `Advance` that results in a state change; no increment occurs for zero-change ticks.
- Snapshots carry `snapshotClock` and `capturedAt` to support replay and timeline correlation.
 - `LayoutDescriptor.id` is computed by the repository as a stable identifier: SHA-256 over canonicalized `.RCD` bytes plus schema version, encoded as a lowercase hex string. This ensures test baselines remain tied to exact content.
#### 5.3.7 Input Validation & Determinism
- `Advance(dt)`: `dt` must be non-negative and clamped to a configured max step (e.g., 1s). If clamped, emit a diagnostics warning; if invalid (e.g., overflow), return `Status{ValidationError, ...}`.
- Determinism: when `EngineConfig::enableDeterministicSeeds` is true, `IRandomProvider` is seeded with a fixed value on `LoadLayout` and on explicit `ResetSeed(...)` commands (future). `IClockService` controls simulated time for tests.
- Re-entrancy: Engine methods are not re-entrant. If a public API is invoked during an in-progress call (e.g., via observer), return `Status{Busy, ...}` and record a diagnostics warning.
#### 5.3.8 Layout Validation
- Repository performs layered validation during `LoadLayout`:
  1) Parse `.RCD` into an AST and normalize (strip comments, canonicalize whitespace/line-endings).
  2) Structural checks (unique IDs/names, ranges, non-overlapping sections, valid references).
  3) Cross-reference resolution (routes ↔ sections, timetable ↔ locos/routes).
  4) Derive metadata (counts, bounds) and compute `LayoutDescriptor.id`.
- Failures return `Status{LayoutError | ValidationError, message}` with a concise message and a diagnostics event containing detailed context.
#### 5.3.9 Command Semantics
- `Pause`/`Resume` are idempotent; issuing the current state returns `Status{Ok, "no-op"}`.
- `Stop` transitions the engine to an inactive state and clears pending timers; `Advance` after `Stop` results in `Status{InvalidCommand, ...}` until `Resume` or a new `LoadLayout`.
- `SetDelayMode` validates `DelaySettings` against configuration bounds and returns `ValidationError` on invalid input.
- `AssignLoco` validates timetable and loco IDs; returns `NotFound` when referencing unknown entities.
### 5.4 Data Flow
...
4. Timers (`WM_TIMER`) call `RailController::OnTimer` with the computed elapsed duration; the controller advances the simulation, consumes resulting `DomainEvent` collections, and schedules repaint/invalidations as required (if `result.delta` is null, the controller skips heavy refresh work). On non-OK `AdvanceOutcome.status`, the controller pauses timers and surfaces errors.
...
#### 5.4.1 Observer Event Ordering
- On successful `LoadLayout`, observers receive exactly one `OnSnapshot` representing the initial world state.
- For each `Advance` that returns `Status::Ok`, observers receive `OnEvents` (may be empty) and no automatic `OnSnapshot` unless the controller explicitly requests one (e.g., for screenshot/export). This keeps steady-state updates lightweight.
- On `Reset`, observers receive no callbacks; the controller is responsible for clearing UI.
- Error paths: when `AdvanceOutcome.status` is non-OK, no `OnEvents` is emitted; the controller surfaces the error and pauses timers.
...
- All public backend/controller entry points return `Status` (or `AdvanceOutcome`), enabling consistent bubbling of validation or runtime failures up to the UI. UI code maps non-OK statuses to modal dialogs, status bar text, or telemetry without needing to inspect exceptions.
- `DiagnosticsModule` registers as an `IObserver` and streams structured diagnostics. By default it writes to the debugger output (`OutputDebugString`) and optional rotating log files. CI runs can route diagnostics to JSON for archival.
...
- **Creation**: `railc_msvc.exe` constructs a `std::unique_ptr<RailController>` and obtains a backend instance via `RailCore::CreateEngine()` returning `std::shared_ptr<IRailEngine>`. Diagnostics and tooling share this pointer, ensuring the backend stays alive while observers remain registered.
...
- **Runtime**: UI dispatches menu/timer events; controller forwards them to the backend and pushes resulting snapshots to delegates. If `AdvanceOutcome.status` is non-OK the controller pauses timers and surfaces the issue through both the delegate and diagnostics sink.
...
## 6. Build & Deployment
- Solution structure (MSVC + MSBuild):
  - `build\msvc\RailControl.vcxproj` builds the UI executable (`railc_msvc.exe`) using OWLNext vendored under `third_party\owlnext`.
  - `build\msvc\RailCore.vcxproj` (new) builds the backend static library (`railcore.lib`) with `/std:c++17`, `/permissive-`, `/Zc:__cplusplus`, and `/W4`. Public headers live under `include\railcore\`.
  - `build\msvc\RailCoreTests.vcxproj` (new) links GoogleTest and `railcore.lib` and runs via Visual Studio Test Explorer or `vstest.console.exe`.
  - Optional: a UI automation harness project for smoke checks (pywinauto-driven or WinAppDriver), invoked from a small test runner.
- Primary entrypoints:
  - `build.bat <Config>` calls the main MSBuild driver (`build.cmd`), builds OWLNext on demand, and compiles all targets for Win32.
  - CI uses GitHub Actions to invoke `build.cmd` directly with `STRICT=1` and optional `COVERAGE=1`.
- Optional future: add CMake presets to side-by-side support `ctest` flows; MSBuild remains the authoritative path.
- Diagnostics dependencies are optional; avoid adding JSON libraries to the core unless explicitly vendored.
- Deployment artifacts remain `railc_msvc.exe`, `railcore.pdb`, resource files, and help assets; packaging scripts gather binaries from the MSVC build tree.
...
- Coverage tooling runs separately for `railcore` (unit) and `railc_msvc.exe` (automation) builds using OpenCppCoverage or VS Enterprise Coverage.
## 8. Migration Plan
1. **Preparation**  
...
5. **Validation**  
   - Establish golden baselines (arrival/departure timelines, scoring) from current MSVC behavior for key layouts and assert equality in backend tests.  
   - Enable backend unit tests in CI.
6. **Cleanup**  
   - Decommission unused globals, update documentation, and remove any residual UI-to-global access paths.
## 9. Risks & Mitigations
