...
- Initialised `POverlapDataInfo[0..49]` to zero in `TLayout::TLayout`; `build_msvc.bat Release` now finishes cleanly.
- Replayed the prior crash script under cdb (`tmp/cdb_release_c374_afterfix.log`); `_SEH` never hits `c0000374` and the run reaches the data-file validation message box, confirming `TLayout::ClearData` no longer frees 0xBAADF00D.
- Sanity breakpoint at `TLayout::ClearData+0x60` (`tmp/cdb_clear_slot.log`) shows the loop writing zero to the first overlap slot instead of deleting a sentinel pointer.
- Next step: trim `OpenDiagnosticsLog` noise for Release builds and investigate the MSCTF first-chance AV so the autorun harness can complete unattended.
## Debugging Notes (14:26)
- Wrapped `OpenDiagnosticsLog` call sites with `DIAG_SETUP_TRACE` so Release builds skip diagnostic file I/O when instrumentation is off.
- Added a fallback in `TLayout::StartNew` that loads the bundled `FAST.RCD` automatically when no data file is configured, allowing autorun scenarios to proceed without the modal warning dialog.
- Verified `build_msvc.bat Release` succeeds after the tidy-up; autorun harness can now reach gameplay without manual input when the INI lacks a data file path.
## Debugging Notes (15:10)
- Debug startup regressed when _RTC_CheckEsp tripped at the end of TMainWindow::SetupWindow; stack diff (ebp-esp = 0x14) pointed to the auto CM_OPTOPTIMI call after window creation.
- SentinelOne still hooks SendMessageA in Debug, leaving the stdcall stack imbalanced; Release uses the optimized /GL path so it slid by.
- Swapped the SendMessage auto-optimise with a direct CMOptOptimi() invocation, bypassing the hooked API. After rebuilding, both Debug and Release start cleanly under cdb (no _RTC_CheckEsp).
